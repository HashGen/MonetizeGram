...
const Setting = require('./models/setting.model'); // --- NEW: Setting model ko yahan bhi import karein
...
// --- FIX: Helper function to escape special characters for Telegram Markdown ---
function escapeMarkdown(text) {
    if (typeof text !== 'string') {
        return '';
    }
    // For parse_mode: 'Markdown', these are the main characters to escape
...
async function processPayment(amount, bot, method = "Unknown") { 
    try { 
        ...
        await bot.sendMessage(SUPER_ADMIN_ID, `💸 *Sale Confirmed\\!* \\(via ${method}\\)\n\nOwner: ${escapeMarkdown(owner.first_name)}\nAmount: \`₹${plan_price.toFixed(2)}\`\nCommission: \`₹${commission.toFixed(2)}\`\nSubscriber: \`${subscriber_id}\``, { parse_mode: 'MarkdownV2' }); 
    } catch (error) { 
        console.error("[processPayment] FATAL CRASH:", error);
        // --- FIX: Escape the error message before sending to prevent parsing errors ---
        const rawErrorMessage = `❌ *CRITICAL ERROR during payment processing for ₹${amount}*\n\n\`\`\`\n${error.toString()}\n\`\`\``;
        await bot.sendMessage(SUPER_ADMIN_ID, rawErrorMessage, { parse_mode: 'Markdown' }); 
    } 
}
