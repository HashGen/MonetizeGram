    ...
    try {
        ...
        if (!payment) {
            if (method.startsWith("Manual")) {
                await bot.sendMessage(SUPER_ADMIN_ID, `❌ *Verification Failed*\nNo pending payment found for amount \`₹${amount}\``, { parse_mode: 'MarkdownV2' });
            }
            return;
        }
        ...
        await bot.sendMessage(subscriber_id, "If you face any problems with the channel or owner, you can report it to the admin here.", { reply_markup: reportKeyboard });
...
async function checkSubscriptions(bot) {
    ...
    for (const sub of expiredSubs) {
        try {
            ...
            const safeChannelName = escapeMarkdownV2(channel.channel_name);
            await bot.sendMessage(userId, `⌛️ *Your Subscription Has Expired*\n\nYour subscription for "*${safeChannelName}*" has expired and you have been removed\\.`, { parse_mode: 'MarkdownV2', reply_markup: renewButton });
            await Subscriber.findByIdAndDelete(sub._id);
            removedCount++;
        } catch (error) {
...
bot.on('message', async (msg) => {
    try {
        ...
        bot.sendMessage(SUPER_ADMIN_ID, `🚨 BOT CRASH in message handler: \n\n\`${safeError}\``, { parse_mode: 'MarkdownV2' });
    }
});
...
bot.on('callback_query', async (callbackQuery) => {
    try {
        ...
        if (fromId === SUPER_ADMIN_ID && data.startsWith('admin_')) {
        ...
        if (data.startsWith('sub_report_')) {
            ...
            await bot.sendMessage(fromId, "Please describe the issue you are facing. Your message will be sent to the admin.");
            return;
        }
        ...
        const safeError = escapeMarkdownV2(error.message);
        bot.sendMessage(SUPER_ADMIN_ID, `🚨 BOT CRASH in callback handler: \n\n\`${safeError}\``, { parse_mode: 'MarkdownV2' });
    }
});
...
async function handleSuperAdminCommands(bot, msg) {
    ...
    if (text === '/viewowners') {
        ...
        const keyboard = owners.map(o => ([{ text: `${escapeMarkdownV2(o.first_name)} (${o.telegram_id}) ${o.is_banned ? '- 🚫 BANNED' : ''}`, callback_data: `admin_inspect_${o._id}` }]));
    ...
    if (text.startsWith('/unban ')) {
        ...
        if (owner) {
            bot.sendMessage(owner.telegram_id, `✅ Good News\\! Your account has been unbanned by the admin\\.`, { parse_mode: 'MarkdownV2' });
            bot.sendMessage(fromId, `✅ Owner ${escapeMarkdownV2(owner.first_name)} has been unbanned.`);
        } else {
            bot.sendMessage(fromId, "Owner not found with that Telegram ID.");
        }
        return true;
    }
    if (text.startsWith('/removesubscriber ')) {
        ...
        try {
            ...
            bot.sendMessage(fromId, `❌ Error removing subscriber: \`${escapeMarkdownV2(error.message)}\``, { parse_mode: 'MarkdownV2' });
        }
        return true;
    }
    return false;
...
async function handleAdminCallback(bot, cbq) {
    ...
    if (action === 'inspect') {
        ...
        const channels = await ManagedChannel.find({ owner_id: objectId });
        let text = `*Inspecting ${escapeMarkdownV2(owner.first_name)}* \\(\`${owner.telegram_id}\`\\)\n\n*Status:* ${owner.is_banned ? '🚫 BANNED' : '✅ Active'}\n*Wallet:* ₹${owner.wallet_balance.toFixed(2)}`;
        const keyboard = channels.map(c => ([{ text: escapeMarkdownV2(c.channel_name), callback_data: `admin_getlink_${c._id}` }]));
        if (!owner.is_banned) {
            keyboard.push([{ text: `🚫 BAN THIS OWNER`, callback_data: `admin_ban_${objectId}` }]);
        }
        keyboard.push([{ text: `⬅️ Back to Owner List`, callback_data: `admin_viewowners` }]);
        bot.editMessageText(text, { chat_id: fromId, message_id: cbq.message.message_id, parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: keyboard } });
    }
    if (action === 'getlink') {
        const channel = await ManagedChannel.findById(objectId);
        const link = await bot.createChatInviteLink(channel.channel_id, { member_limit: 1 });
        bot.sendMessage(fromId, `Here is the one\\-time inspection link for *${escapeMarkdownV2(channel.channel_name)}*:\n\n${link.invite_link}`, { parse_mode: 'MarkdownV2' });
    }
    if (action === 'ban') {
        const owner = await Owner.findByIdAndUpdate(objectId, { is_banned: true, banned_at: new Date() });
        await ManagedChannel.deleteMany({ owner_id: objectId });
        bot.sendMessage(owner.telegram_id, `⚠️ *Your Account Has Been Banned* ⚠️\n\nThis is due to a violation of our terms\\. Your channels have been removed and withdrawals are disabled\\.\n\nPlease contact support: @${SUPER_ADMIN_USERNAME}`, { parse_mode: 'MarkdownV2' });
        bot.editMessageText(`✅ Owner ${escapeMarkdownV2(owner.first_name)} has been banned.`, { chat_id: fromId, message_id: cbq.message.message_id });
    }
    if (action === 'viewowners') {
        const owners = await Owner.find({}).sort({ created_at: -1 }).limit(10);
        const keyboard = owners.map(o => ([{ text: `${escapeMarkdownV2(o.first_name)} (${o.telegram_id}) ${o.is_banned ? '- 🚫 BANNED' : ''}`, callback_data: `admin_inspect_${o._id}` }]));
        bot.editMessageText("Select an owner to inspect:", { chat_id: fromId, message_id: cbq.message.message_id, reply_markup: { inline_keyboard: keyboard } });
    }
    if (action === 'superhelpsection') {
        ...
        await bot.editMessageText(helpContent[objectId], { chat_id: fromId, message_id: cbq.message.message_id, parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "admin_superhelp" }]] } });
    }
    if (action === 'superhelp') {
        const keyboard = { inline_keyboard: [[{ text: "🚨 Manual Verification", callback_data: "admin_superhelpsection_verification" }], [{ text: "👑 Admin Dashboard Explained", callback_data: "admin_superhelpsection_dashboard" }], [{ text: "🔑 Moderation Commands", callback_data: "admin_superhelpsection_moderation" }],] };
