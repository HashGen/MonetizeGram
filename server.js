...
async function processPayment(amount, bot, method = "Unknown") {
    try {
        ...
        await bot.sendMessage(owner.telegram_id, ownerMessage);
        const adminMessage = `💸 *Sale Confirmed!* (via ${method})\n\nOwner: ${owner.first_name}\nAmount: \`₹${plan_price.toFixed(2)}\`\nCommission: \`₹${commission.toFixed(2)}\`\nSubscriber: \`${subscriber_id}\``;
        await bot.sendMessage(SUPER_ADMIN_ID, adminMessage, { parse_mode: 'Markdown' });
    } catch (error) {
        ...
        await bot.sendMessage(SUPER_ADMIN_ID, finalMessage, { parse_mode: 'Markdown' });
    }
}
...
bot.on('message', async (msg) => {
    try {
        ...
        if (state && state.awaiting === 'report_reason') {
            ...
            await bot.sendMessage(SUPER_ADMIN_ID, reportMessage, { parse_mode: 'Markdown' });
