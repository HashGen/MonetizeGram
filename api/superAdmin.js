...
const Report = require('../models/report.model'); // Import Report model
...
module.exports = function(bot) {
    ...
    router.get('/stats', async (req, res) => {
        try {
            const [ownerCount, channelCount, revenueAggregation, commissionAggregation, paidOutAggregation, pendingPayoutAggregation] = await Promise.all([
            ...
            res.json({ totalOwners: ownerCount, totalChannels: channelCount, totalRevenue: revenueAggregation[0]?.total || 0, totalCommission: commissionAggregation[0]?.total || 0, totalPaidOut: paidOutAggregation[0]?.total || 0, pendingPayouts: pendingPayoutAggregation[0]?.total || 0 });
    ...
    // --- THIS IS THE FIX for "Failed to load channels" ---
    router.get('/owners/:id/channels', async (req, res) => {
        try {
            const channels = await ManagedChannel.find({ owner_id: req.params.id });
            res.json(channels);
        } catch (error) {
            res.status(500).json({ error: "Failed to get owner's channels" });
        }
    });
    
    // --- WITHDRAWAL MANAGEMENT ---
    router.get('/withdrawals', async (req, res) => { /* ... unchanged ... */ });
    router.post('/withdrawals/:id/approve', async (req, res) => { /* ... unchanged ... */ });
    router.post('/withdrawals/:id/reject', async (req, res) => { /* ... unchanged ... */ });

    // --- MODERATION ---
    router.post('/owners/:id/ban', async (req, res) => { /* ... unchanged ... */ });

    // --- THIS IS THE FIX for "Get Invite Link" button ---
    router.post('/channels/:id/inspect', async (req, res) => {
        try {
            const channel = await ManagedChannel.findById(req.params.id);
            if (!channel) return res.status(404).send("Channel not found");
            const link = await bot.createChatInviteLink(channel.channel_id, { member_limit: 1 });
            await bot.sendMessage(process.env.SUPER_ADMIN_ID, `Here is the one-time inspection link for channel *${channel.channel_name}*:\n\n${link.invite_link}`, { parse_mode: 'Markdown' });
            res.json({ message: "Success! An invite link has been sent to your Telegram chat." });
        } catch (error) {
            console.error("Inspect link error:", error);
            res.status(500).json({ error: "Could not create inspect link. Make sure the bot is still an admin in the channel." });
        }
    });
