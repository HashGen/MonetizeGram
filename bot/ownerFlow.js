...
async function handleOwnerMessage(bot, msg) {
    ...
    if (text.startsWith('/')) {
        delete userStates[fromId]; // If user sends a command, cancel any pending state
    }
...
async function handleOwnerCallback(bot, cbq) {
    ...
    // --- THIS IS THE FIX ---
    // Clear any pending state when a button is clicked
...
// ... (Rest of the file is unchanged, but provided below for completeness)
...
module.exports = { handleOwnerMessage, handleOwnerCallback, initialize };
